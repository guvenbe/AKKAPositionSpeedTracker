...
public class MainParallel {
    ...
    public static void main(String[] args) {
        ...
        Source<String, NotUsed> source = Source.repeat("go")
                .throttle(1, Duration.ofSeconds(10)); //Number of item to repeat every 10 seconds
        ...
        Flow<String, Integer, NotUsed> vehicleIds = Flow.of(String.class)
                .mapConcat(id -> List.of(1,2,3,4,5,6,7,8));
        ...
        Flow<Integer, VehiclePositionMessage, NotUsed> vehiclePosition =
                Flow.of(Integer.class).mapAsyncUnordered(8, vehicleId -> {
        ...
        Flow<VehiclePositionMessage, VehicleSpeed, NotUsed> vehicleSpeeds = Flow.of(VehiclePositionMessage.class).map(vpm -> {
            UtilityFunctions utilityFunctions = new UtilityFunctions();
        ...
        Flow<VehicleSpeed, VehicleSpeed, NotUsed> speedFilter =
                Flow.of(VehicleSpeed.class).filter(speed -> speed.getSpeed() > 95);
...
//                    SinkShape<VehicleSpeed> out = builder.add(sink);
                    ...
                    for (int i = 0; i < 8; i++) {
                        builder.from(balance)
                                .via(builder.add(vehiclePosition.async()))
                                .toFanIn(merge);
                    }
        ...
        result.whenComplete((value, throwable)->{
            if(throwable != null){
                ...
                System.out.println("Vehicle " + value.getVehicleId() + " was going a speed of " + value.getSpeed());
            }
